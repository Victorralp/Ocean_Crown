const en = {
  // Navigation
  navigation: {
    tracking: "Tracking",
    search: "Search",
    login: "myMSC",
    language: "Language"
  },
  
  // Menu items
  menu: {
    solutions: "Solutions",
    industries: "Industries",
    ebusiness: "eBusiness",
    sustainability: "Sustainability",
    aboutUs: "About Us",
    schedules: "Schedules"
  },
  
  // Hero section
  hero: {
    leaderIn: "OCEAN CROWN",
    shippingLogistics: "MULTILINK ENTERPRISE LTD",
    trackingTab: "TRACKING",
    schedulesTab: "SCHEDULES",
    contactsTab: "CONTACTS",
    containerBol: "Container / Bill of Lading Number",
    bookingNumber: "Booking Number",
    searchPlaceholder: "Search our services and solutions...",
    searchButton: "Search"
  },
  
  // Example component
  example: {
    title: "Using Translations Across Components",
    text: "This text is coming from the translation system. Each component can use the translation hook to display content in the selected language."
  },
  
  // Services Component
  services: {
    sectionTitle: "Our Services",
    sectionDescription: "Ocean Crown delivers comprehensive shipping and logistics services designed to optimize your supply chain and enhance your business performance.",
    learnMore: "Learn More",
    contactUs: "Request a Customized Solution",
    
    ocean: {
      title: "Ocean Freight",
      description: "Reliable ocean freight services connecting major ports worldwide with scheduled departures and arrivals.",
      feature1: "FCL (Full Container Load) shipping",
      feature2: "LCL (Less than Container Load) options",
      feature3: "Specialized equipment for oversized cargo"
    },
    
    land: {
      title: "Land Transport",
      description: "Comprehensive road and rail transport solutions for regional and international distribution.",
      feature1: "FTL and LTL truck services",
      feature2: "Intermodal rail transport",
      feature3: "Door-to-door solutions"
    },
    
    warehousing: {
      title: "Warehousing",
      description: "Strategic warehousing facilities in key locations offering storage, distribution, and value-added services.",
      feature1: "Short and long-term storage options",
      feature2: "Inventory management",
      feature3: "Order fulfillment"
    },
    
    global: {
      title: "Global Logistics",
      description: "End-to-end supply chain solutions tailored to your business needs and global requirements.",
      feature1: "Supply chain consulting",
      feature2: "International shipping expertise",
      feature3: "Multi-modal transportation"
    },
    
    customs: {
      title: "Customs Brokerage",
      description: "Streamlined customs clearance services to navigate complex international trade regulations.",
      feature1: "Import/export documentation",
      feature2: "Customs compliance consulting",
      feature3: "Duties and taxes management"
    },
    
    documentation: {
      title: "Documentation Services",
      description: "Expert handling of all shipping documentation to ensure compliance and smooth operations.",
      feature1: "Bills of lading preparation",
      feature2: "Certificate of origin management",
      feature3: "Electronic documentation"
    },
    
    featured: {
      title: "Ocean Crown Integrated Solutions",
      description: "Our integrated logistics platform combines multiple services into a seamless supply chain solution, offering visibility, efficiency, and reliability.",
      item1: {
        title: "Supply Chain Visibility",
        description: "Real-time tracking and monitoring of your shipments with detailed status updates and exception alerts."
      },
      item2: {
        title: "Performance Analytics",
        description: "Comprehensive reporting and analytics to identify optimization opportunities in your supply chain."
      }
    }
  },
  
  // Stats Component
  stats: {
    title: "Global Reach, Local Expertise",
    description: "With a robust global network and deep local knowledge, we connect businesses across continents.",
    years: "Years of Experience",
    countries: "Countries Served",
    clients: "Active Clients",
    shipments: "Annual Shipments"
  },
  
  // Industries Component
  industries: {
    title: "Industries We Serve",
    description: "Specialized solutions for diverse business sectors, each with unique supply chain requirements.",
    automotive: {
      title: "Automotive",
      description: "Just-in-time logistics for automotive manufacturers and suppliers."
    },
    retail: {
      title: "Retail & Consumer",
      description: "Omnichannel fulfillment solutions for retailers and consumer goods companies."
    },
    pharma: {
      title: "Healthcare & Pharma",
      description: "Temperature-controlled and compliant logistics for the healthcare industry."
    },
    manufacturing: {
      title: "Manufacturing",
      description: "Streamlined logistics for manufacturing supply chains and production lines."
    },
    energy: {
      title: "Energy & Resources",
      description: "Specialized services for the energy sector and resource industries."
    },
    tech: {
      title: "Technology",
      description: "Fast, secure logistics for high-value technology products and components."
    }
  },
  
  // About Us Component
  aboutUs: {
    title: "About Ocean Crown",
    subtitle: "Your trusted logistics partner since 2012, connecting businesses across continents.",
    ourStory: {
      title: "Our Story",
      paragraph1: "Ocean Crown Multi-Links Enterprises Ltd was established in 2012 in Lagos, Nigeria, with a vision to revolutionize the logistics and shipping industry in West Africa through exceptional service delivery and operational excellence.",
      paragraph2: "Over the years, we have expanded our services and geographical reach, forming Global Ocean Crown Oil & Transport Ltd in 2018 to specialize in oil sector logistics and transportation services, enhancing our capabilities in Nigeria's energy market.",
      paragraph3: "Today, we are recognized as a leading logistics provider in Nigeria, offering comprehensive shipping, clearing, forwarding, and transportation services to clients across various industries."
    }
  },
  
  // Footer
  footer: {
    about: {
      title: "About Ocean Crown",
      description: "A global leader in shipping and logistics services with over 30 years of excellence in international trade."
    },
    quickLinks: "Quick Links",
    contactUs: "Contact Us",
    subscribe: {
      title: "Subscribe to Our Newsletter",
      placeholder: "Your email address",
      button: "Subscribe",
      description: "Stay updated with the latest industry news and company updates."
    },
    copyright: "© 2023 Ocean Crown. All rights reserved.",
    privacyPolicy: "Privacy Policy",
    termsOfService: "Terms of Service"
  },
  
  // Contact Component
  contact: {
    title: "Contact Us",
    info: {
      title: "Contact Information",
      address: {
        title: "Address",
        text: "29 Payne Crescent, Apapa, Lagos, Nigeria"
      },
      phone: {
        title: "Phone",
        text: "+234 123 456 7890"
      },
      email: {
        title: "Email",
        text: "info@oceancrown.com"
      }
    },
    form: {
      title: "Send Us a Message",
      name: "Your Name",
      email: "Your Email",
      phone: "Your Phone",
      subject: "Subject",
      message: "Your Message",
      submit: "Send Message",
      success: "Thank you for your message. We'll get back to you soon!"
    }
  }
};

export default en; 